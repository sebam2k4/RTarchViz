...
class PostModelTest(TestCase):
    ...
    def test_get_author_method_when_no_name(self):
        self.user = get_user_model().objects.create(
            username='test_user', email='test_user@gmail.com')
        post = Post.objects.create(title='1-title', content='1-content',
                                   status='published', author=self.user)
        full_name = post.get_author()
...
# make assertion that doesn't exist first
    def test_one_published_post(self):
        Post.objects.create(title='1-title', content='1-content',
    ...
    def test_one_draft_post(self):
        Post.objects.create(title='2-title', content='2-content',
    ...
    def test_two_published_posts(self):
        Post.objects.create(title='3-title', content='3-content',
                            author=self.user, status='published')
        Post.objects.create(title='4-title', content='4-content',
                            author=self.user, status='published')
        response = self.client.get('/blog/')
    ...
    def test_one_published_and_one_draft_posts(self):
        Post.objects.create(title='5-title', content='5-content',
                            author=self.user, status='published')
        Post.objects.create(title='6-title', content='6-content',
                            author=self.user, status='draft')
        response = self.client.get('/blog/')
    ...
    def setUpTestData(cls):
        ...
        author = get_user_model().objects.create(
            username='author1', email='author1@gmail.com')
        number_of_posts = 11
        for post_num in range(number_of_posts):
            Post.objects.create(author=author, title='title {0}'.format(post_num),
            content='content {0}'.format(post_num), status='published'
            )
...
class BlogPostDetailViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            title='1-title', content='1-content', status='published',
            slug='1-title', author=self.user
        )
