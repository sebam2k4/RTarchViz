...
class Order(models.Model):
    ...
    product_count = models.IntegerField(editable=False, blank=False,
                                        null=False, default=0)
    total_amount = models.DecimalField(editable=False, max_digits=8,
...
class OrderProduct(models.Model):
    ...
    # TO STRING METHOD
    def __unicode__(self):
        return "{0} from (Order #{1}) - seller: {2}".format(
            self.product.name, self.order_id, self.product.seller.username)
...
class PurchaseHistoryManager(models.Manager):
    ...
    def purchased_products_history(self, user):
        """ get user's purchased products """
        return self.get_queryset().filter(buyer_id=user.id).order_by(
            '-purchase_date')

    def sold_products_history(self, user):
        """ get user's sold products """
        return self.get_queryset().filter(seller_id=user.id).order_by(
            '-purchase_date')
