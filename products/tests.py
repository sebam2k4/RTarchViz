...
class NewProductViewTest(TestCase):
  ...
  def setUp(self):
    #Create one user
    test_user1 = get_user_model().objects._create_user(email='testuser1@gmail.com', username='testuser1', password='testtest1', is_staff=False, is_superuser=False) 
    test_user1.save()
  ...
  def test_logged_in_uses_correct_template(self):
    login = self.client.login(email='testuser1@gmail.com', password='testtest1')
    response = self.client.get(reverse('new_product'))
...
class EditProductViewTest(TestCase):
  ...
  def setUp(self):
    ...
    test_user1 = get_user_model().objects._create_user(email='testuser1@gmail.com', username='testuser1', password='testtest1', is_staff=False, is_superuser=False) 
    test_user1.save()
    self.product = Product.objects.create(id=1, name='1-name', price=10, seller=test_user1)

  def test_redirect_if_not_logged_in(self):
    response = self.client.get(self.product.get_edit_product_url())
    self.assertRedirects(response, '/accounts/login/?next=/products/{0}--{1}/edit/'.format(self.product.slug, self.product.id))
...
class DeleteProductViewTest(TestCase):
  ...
  def test_redirect_if_not_logged_in(self):
    ...
    self.assertRedirects(response, '/accounts/login/?next=/products/{0}--{1}/delete/'.format(self.product.slug, self.product.id))